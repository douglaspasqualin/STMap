...
typedef struct stm_tx { /* Transaction descriptor */
    ...
    unsigned long number_aborts;
    unsigned long total_tx;
    unsigned long rset_size;
    unsigned long wset_size;
#endif
#if INSTRUMENT == ONLINE
    int totalAddresses;
#endif
...
#define GET_ADDR_SHARE(a)               (map_access + LOCK_IDX(a))
    ...
    address_entry map_access[LOCK_ARRAY_SIZE];
//    pthread_mutex_t comm_matrix_mutex;
    double **comm_matrix; 
   // access_type access_per_thread[MAX_COMM_THREADS];
    uint32_t sampleInterval; //when get memory access, every "sampleInterval" access
    uint8_t needMapping;    
#if INSTRUMENT == ONLINE
    pthread_t pthreads[MAX_COMM_THREADS];
...
typedef struct {
    ...
    unsigned long distinct_addr;
} instrument_data_t;

static instrument_data_t instrument_data;
#endif
...
//for avoiding using math.h and linking with -lm
static inline  
...
do_thread_mapping() {
    ...
    //printf("New thread mapping: ");
    for (i = 0; i<_tinystm.total_threads;i++) {
    // printf("%d ", sol->sigma[i]);
        thread_pin_to_cpu(pthreads[i], sol->sigma[i]);
    }
    //    printf("\n");
...
static INLINE void
save_instrumentation_data(stm_tx_t *tx) {
#if INSTRUMENT == METHOD1 || INSTRUMENT == ONLINE
    if (needMapping && tx->int_thread_id == THREAD_TO_INSTRUMENT) {
        ATOMIC_FETCH_INC_FULL(&instrument_data.commits);
        ATOMIC_FETCH_INC_FULL(&instrument_data.total_tx);
        
        ATOMIC_FETCH_ADD_FULL(&instrument_data.total_tx, tx->total_tx);
        ATOMIC_FETCH_ADD_FULL(&instrument_data.number_aborts, tx->number_aborts);
        
        ATOMIC_FETCH_ADD_FULL(&instrument_data.rset_size, tx->r_set.nb_entries);
        ATOMIC_FETCH_ADD_FULL(&instrument_data.rset_size, tx->rset_size);
        
        ATOMIC_FETCH_ADD_FULL(&instrument_data.wset_size, tx->w_set.nb_entries);
        ATOMIC_FETCH_ADD_FULL(&instrument_data.wset_size, tx->wset_size);
...
static inline void
print_statistics() {
    printf("\n\n### Time do to thread mapping, printing statistics\n");
    printf("Application: %s\n", application_name);
    printf("Threads: %d\n", _tinystm.total_threads);
    printf("Commits: %lu\n", instrument_data.commits);
    printf("Aborts: %lu\n", instrument_data.number_aborts);
    //printf("Max Retries: %lu\n", retries);
    
    double ca = instrument_data.commits + instrument_data.number_aborts;
    if (ca > 0) {
        printf("Commit ratio: %.2f\n", instrument_data.commits / ca);
        printf("Abort ratio: %.2f\n", instrument_data.number_aborts / ca);
    }

    printf("Total transactions (A + C): %lu\n", instrument_data.total_tx);
    if (instrument_data.total_tx > 0) {
        printf("Average size of read-set: %.2f\n", (double) instrument_data.rset_size / instrument_data.total_tx);
        printf("Average size of write-set: %.2f\n", (double) instrument_data.wset_size / instrument_data.total_tx);
    } else {
        printf("No transaction active!\n");
    }
    
    printf("Distinct address accessed: %lu\n", instrument_data.distinct_addr);
    printf("Total address accessed: %lu\n", instrument_data.total_addr);
...
address_entry* getAddrEntry(volatile stm_word_t *addr) {
    ATOMIC_FETCH_INC_FULL(&instrument_data.total_addr);
    
    unsigned long int address = (stm_word_t) addr; //>> CACHE_BLOCK_BITS;
    address_entry *elem = GET_ADDR_SHARE(address);
   
    if (elem->address == 0) {
        ATOMIC_FETCH_INC_FULL(&instrument_data.distinct_addr);
...
static inline void
do_check_comm(volatile stm_word_t *addr, stm_tx_t *tx) {
...
check_comm(volatile stm_word_t *addr, stm_tx_t *tx, int access_type) {
    if (needMapping) { //Only sample memory if thread mapping still is necessary
        tx->addressesSample++;
        if (tx->addressesSample >= sampleInterval) {
        ...
        //Only monitor thread one. Assuming that all threads have the same amount of tasks
        if (tx->int_thread_id == THREAD_TO_INSTRUMENT && tx->totalAddresses >= mapping_interval) {
            //printf("Total accesses: %d, time to do thread mapping\n", tx->totalAddresses);
            needMapping = 0;
            save_instrumentation_data(tx);
            //do_thread_mapping();
            print_statistics();
        }
#endif        
...
print_instrumentation_data() {
    ...
    //Save communication matrix in a csv file
    char* fileName;
    int threads = _tinystm.total_threads;
    asprintf(&fileName, "%s_%d_%d.csv", application_name, threads, sampleInterval);
    if (fileExists(fileName)) {
        i = 0;
        do {
            i++;
            asprintf(&fileName, "%s_%d_%d_%d.csv", application_name, threads, sampleInterval, i);
    ...
    for (i = threads-1; i >= 0; i--) {
        for (j = 0; j < threads; j++) {
            ...
            printf("%.0lf", comm_matrix[i][j]);
            fprintf(fPtr, "%.0lf", comm_matrix[i][j]);
            //printf("%lu", comm_matrix.matrix[i * comm_matrix.nthreads + j]);
            //fprintf(fPtr, "%lu", comm_matrix.matrix[i * comm_matrix.nthreads + j]);
        }
        printf("\n");
        fprintf(fPtr, "\n");
    }
    ...
    //Verify total of distinct addresses accessed
    total_addr = 0;
...
stm_quiesce_init(void) {
 ...
 /*   if (pthread_mutex_init(&comm_matrix_mutex, NULL) != 0) {
    ...
    if (getenv("SAMPLE_INTERVAL") == NULL) {
        printf("SAMPLE_INTERVAL must be defined.\n");
        exit(EXIT_FAILURE);
    }
    sampleInterval = atoi(getenv("SAMPLE_INTERVAL"));
    needMapping = 1;
...
#if INSTRUMENT == ONLINE
    if (getenv("MAPPING_INTERVAL") == NULL) {
        printf("MAPPING_INTERVAL must be defined\n");
        exit(EXIT_FAILURE);
    }    
    mapping_interval = atoi(getenv("MAPPING_INTERVAL"));
    
    /* Treematch initialization */
    topology = tm_get_local_topology_with_hwloc();
    /* Only 1 mapping per core*/
    int oversubfalse = 0;
    tm_enable_oversubscribing(topology, oversubfalse);
#endif
#endif
...
//TIMER_READ(startTime);
 ...
 //   print_cores();
    ...
    read_map_file("topomatch.txt", atoi(getenv("MAP_LINE")));
...
dropped:
    ...
    if (needMapping && tx->int_thread_id == THREAD_TO_INSTRUMENT) {
        tx->number_aborts++;
        tx->total_tx++;
        tx->rset_size += tx->r_set.nb_entries;
        tx->wset_size = tx->w_set.nb_entries;
/*        ATOMIC_FETCH_INC_FULL(&instrument_data.number_aborts);
        ATOMIC_FETCH_INC_FULL(&instrument_data.total_tx);
        ATOMIC_FETCH_ADD_FULL(&instrument_data.rset_size, tx->r_set.nb_entries);
        ATOMIC_FETCH_ADD_FULL(&instrument_data.wset_size, tx->w_set.nb_entries); 
 */
