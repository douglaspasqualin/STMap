...
check_comm(volatile stm_word_t *addr, stm_tx_t *tx, int access_type) {
    if (needMapping) { //Only sample memory if thread mapping still is necessary
        tx->addressesSample++;
        if (tx->addressesSample >= SAMPLE_INTERVAL) {
...
print_instrumentation_data() {
    ...
    asprintf(&fileName, "%s_%d_%d.csv", application_name, threads, SAMPLE_INTERVAL);
    if (fileExists(fileName)) {
        i = 0;
        do {
            i++;
            asprintf(&fileName, "%s_%d_%d_%d.csv", application_name, threads, SAMPLE_INTERVAL, i);
