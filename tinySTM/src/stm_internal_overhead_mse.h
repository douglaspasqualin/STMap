...
static inline int
enable_mapping(stm_tx_t *tx) {
    //    printf("Commit ratio: %.2f\n", cr);
    //    printf("Abort ratio: %.2f\n", ar);
    unsigned long addrs = get_total_addr_hash_table();
    printf("Distinct addresses: %lu\n", addrs);
    //    int enable;
    if (addrs <= 10000) {
        return 1;
    } else {
        double total_tx = tx->stat_commits + tx->stat_aborts;
        double cr, ar;
        if (total_tx > 0) {
            cr = tx->stat_commits / total_tx;
            ar = tx->stat_aborts / total_tx;
        } else {
            cr = 0.0;
            ar = 0.0;
        }
    ...
    //    printf("#E\t%s\t%d\t%d\n", application_name, _tinystm.total_threads, enable);
    //    return enable;
}
...
check_comm(volatile stm_word_t *addr, stm_tx_t *tx, int access_type) {
    if (needMapping) { //Only sample memory if thread mapping still is necessary
        ...
        //Only monitor one thread. Assuming that all threads have the same amount of tasks
        if (tx->int_thread_id == THREAD_TO_INSTRUMENT
...
stm_quiesce_init(void) {
 ...
 /*   if (pthread_mutex_init(&comm_matrix_mutex, NULL) != 0) {
    ...
    /* TopoMatch initialization */
    if (getenv("TOPOLOGY_HWLOC_XML") != NULL) {
        topology = tm_load_topology(getenv("TOPOLOGY_HWLOC_XML"), TM_FILE_TYPE_XML);
        tm_optimize_topology(&topology);
    } else {
        topology = tm_get_local_topology_with_hwloc();
    }
    /* Only 1 mapping per core*/
    tm_enable_oversubscribing(topology, 1);
#endif
#endif
