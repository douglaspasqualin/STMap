...
MAIN(argc, argv) {
    ...
    TM_STARTUP(nb_threads, "redblacktree");
    ...
    if (getenv("STM_BIND_THREAD") != NULL) {
        printf("#T\t%s\t%lf\t%d\t%s\n", "Redblacktree", TIMER_DIFF_SECONDS(start, stop), nb_threads, getenv("STM_BIND_THREAD"));
    }
    if (getenv("STM_STATS") != NULL) {
        ...
        printf("#C\t%s\t%lu\t%d\t%s\n", "Redblacktree", commits, nb_threads, getenv("STM_BIND_THREAD"));
        printf("#A\t%s\t%lu\t%d\t%s\n", "Redblacktree", aborts, nb_threads, getenv("STM_BIND_THREAD"));
    }
